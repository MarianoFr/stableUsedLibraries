...
class TinyGsmSim7000SSL
 ...
 public:
   ...
   */
 public:
 //Mariano Inicio: consultar el servidor NTP y sincronizar la hora del modem
  bool getNTPtime()
  {
    bool res = false;
    sendAT(GF("+CNTP"));
    if(1 == waitResponse(CONNECT_TO, GFP("+CNTP: 1"), GFP("+CNTP: 64")))
    {
      res = true;
    }
    else
...
#endif
    waitResponse();
	//Mariano Inicio: setea modos GSM y LTE
    sendAT(GF("+CNMP=51"));
    waitResponse();
    sendAT(GF("+CMNB=3"));//CAT-m y NB IOT
    waitResponse();
    //Mariano Fin
 ...
 protected:
  bool gprsConnectImpl(const char* apn, const char* user = NULL,
    ...
    sendAT(GF("+CNTPCID=1"));
    waitResponse();
    sendAT(GF("+CNTP=\"216.239.35.12\",-12"));//216.239.35.12
    waitResponse();
    //Mariano Fin
 ...
 protected:
  int modemConnect(const char* host, uint16_t port, uint8_t mux,
    ...
    //configure transparent parameters, see if they have any impact on CA commands
    sendAT(GF("+CACFG=\"TRANSWAITTM\",30"));
    waitResponse();
    sendAT(GF("+CACFG=\"KEEPALIVE\",1"));
    waitResponse();
    sendAT(GF("+CACFG=\"TIMEOUT\",30"));
    waitResponse();
    ...
    waitResponse();
    sendAT(GF("+CASSLCFG="),mux,GF(",\"timeout\","),CONNECT_TO);
    waitResponse();
    if (ssl) {
