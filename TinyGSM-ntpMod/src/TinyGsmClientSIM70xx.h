...
class TinyGsmSim70xx : public TinyGsmModem<TinyGsmSim70xx<modemType>>,
 ...
 public:
  ...
  //Mariano Inicio
  bool testAT(uint32_t timeout_ms = 20000L) {
 ...
 protected:
  ...
  bool GPSStateImpl() {
    ...
    if (thisModem().waitResponse(GF(GSM_NL "+CGNSPWR: 1")) != 1) { return false; }
    return true;
  }
  //Mariano  Fin
  bool disableGPSImpl() {
  ...
  bool getGPSImpl(float* lat, float* lon, float* speed = 0, float* alt = 0,
    ...
    if (thisModem().streamGetIntBefore(',') == 1) {  // fix status
      ...
      secondWithSS = thisModem().streamGetFloatBefore(',');  // 6 digit second with subseconds
	  //Mariano Inicio
      //Return only when a second's passed
      if(prev_secs==secondWithSS)
      {
      ...
      ispeed = thisModem().streamGetFloatBefore(','); // Speed Over Ground. Unit is knots.
      ...
      iaccuracy = thisModem().streamGetFloatBefore(','); // Horizontal Dilution Of Precision
